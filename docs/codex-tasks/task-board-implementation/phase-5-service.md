...
## File 1: `src/task_board_service/services/task_manager.py`

Create this file. This is the core business logic — no FastAPI imports. Every endpoint's behavior is implemented here. Routers are thin wrappers that call these methods.
...
class TaskManager:
    ...
    async def _release_escrow(self, escrow_id: str, recipient_id: str) -> None:
        ...
        """
        await self._central_bank_client.release_escrow(
            escrow_id=escrow_id,
            recipient_account_id=recipient_id,
        )

    async def _try_release_escrow(
        self, task_id: str, escrow_id: str, recipient_id: str
    ) -> None:
        """
        ...
        if task["status"] == "open":
            ...
            if bidding_deadline is not None:
                deadline_dt = datetime.fromisoformat(
                    bidding_deadline.replace("Z", "+00:00")
                )
                if now >= deadline_dt:
                    expired_at = _now_iso()
                    cursor = self._db.execute(
            ...
            if execution_deadline is not None:
                deadline_dt = datetime.fromisoformat(
                    execution_deadline.replace("Z", "+00:00")
                )
                if now >= deadline_dt:
                    expired_at = _now_iso()
                    cursor = self._db.execute(
            ...
            if review_deadline is not None:
                deadline_dt = datetime.fromisoformat(
                    review_deadline.replace("Z", "+00:00")
                )
                if now >= deadline_dt:
    ...
    async def _evaluate_deadlines_batch(
    ...
    async def _validate_jws_token(
        ...
        if payload["action"] != expected_action:
            raise ServiceError(
                "INVALID_PAYLOAD",
                f"Expected action '{expected_action}', got '{payload['action']}'",
                400,
                {},
            )
    ...
    async def create_task(self, task_token: str, escrow_token: str) -> dict[str, Any]:
        ...
        deadline_seconds: object = payload["deadline_seconds"]
        ...
        if not isinstance(spec, str) or len(spec) < 1 or len(spec) > 10000:
            raise ServiceError(
                "INVALID_PAYLOAD",
        ...
        escrow_result = await self._central_bank_client.lock_escrow(escrow_token)
        ...
        try:
            self._db.execute(
                ...
                """,
                (
                    task_id, poster_id, title, spec, int(reward),
                    int(bidding_deadline_seconds), int(deadline_seconds),
                    int(review_deadline_seconds),
    ...
    async def submit_bid(self, task_id: str, token: str) -> dict[str, Any]:
        ...
        # Step 7d: Validate proposal
        if "proposal" not in payload:
            raise ServiceError("INVALID_PAYLOAD", "Missing required field: proposal", 400, {})

        proposal: str = payload["proposal"]
        if not isinstance(proposal, str) or len(proposal) < 1:
            raise ServiceError("INVALID_PAYLOAD", "Proposal must be a non-empty string", 400, {})

        if len(proposal) > 10000:
            raise ServiceError(
                "INVALID_PAYLOAD",
                "Proposal must not exceed 10,000 characters",
                400,
                {},
            )
        ...
        try:
            self._db.execute(
                "INSERT INTO bids (bid_id, task_id, bidder_id, proposal, submitted_at) VALUES (?, ?, ?, ?, ?)",
    ...
    async def list_bids(
        self, task_id: str, auth_token: str | None
    ) -> dict[str, Any]:
        """
        ...
        cursor = self._db.execute(
            "SELECT bid_id, bidder_id, proposal, submitted_at FROM bids WHERE task_id = ? ORDER BY submitted_at",
            (task_id,),
        )
        rows = cursor.fetchall()
    ...
    async def accept_bid(
        self, task_id: str, bid_id: str, token: str
    ) -> dict[str, Any]:
        """
    ...
    async def upload_asset(
        ...
        self._db.execute(
            """
            INSERT INTO assets (asset_id, task_id, uploader_id, filename, content_type, size_bytes, uploaded_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (asset_id, task_id, signer_id, filename, content_type, len(file_content), uploaded_at),
        )
        self._db.commit()
    ...
    async def list_assets(self, task_id: str) -> dict[str, Any]:
        ...
        cursor = self._db.execute(
            """
            SELECT asset_id, uploader_id, filename, content_type, size_bytes, uploaded_at
    ...
    async def get_asset(
        ...
        cursor = self._db.execute(
            "SELECT asset_id, filename, content_type FROM assets WHERE asset_id = ? AND task_id = ?",
            (asset_id, task_id),
        )
        row = cursor.fetchone()
        if row is None:
    ...
    async def submit_deliverable(self, task_id: str, token: str) -> dict[str, Any]:
        ...
        if task["status"] != "accepted":
            raise ServiceError(
                "INVALID_STATUS",
                f"Cannot submit deliverable for task in '{task['status']}' status, must be 'accepted'",
                409,
                {},
            )
    ...
    async def dispute_task(self, task_id: str, token: str) -> dict[str, Any]:
        ...
        """
        # Steps 4-7a: Verify JWS, validate action
        payload = await self._validate_jws_token(token, "dispute_task")
        signer_id: str = payload["_signer_id"]
    ...
    async def record_ruling(self, task_id: str, token: str) -> dict[str, Any]:
        ...
        """
        # Steps 4-7a: Verify JWS, validate action
        payload = await self._validate_jws_token(token, "record_ruling")
        signer_id: str = payload["_signer_id"]
        ...
        if not isinstance(ruling_id, str) or len(ruling_id) < 1:
            raise ServiceError("INVALID_PAYLOAD", "ruling_id must be a non-empty string", 400, {})
        ...
        self._db.execute(
            ...
            WHERE task_id = ?
            """,
            (ruled_at, ruling_id, int(worker_pct), ruling_summary, task_id),
        )
        self._db.commit()
...
## File 2: `src/task_board_service/services/__init__.py`
...
The implementation follows the 13-level error precedence from the auth spec exactly. Within each public method, the order is:

1. **JWS verification** via `_validate_jws_token` — handles INVALID_JWS (step 4), IDENTITY_SERVICE_UNAVAILABLE (step 5), FORBIDDEN for invalid signatures (step 6), and INVALID_PAYLOAD for wrong/missing action (step 7).
2. **Payload field validation** — required fields, format checks (step 7).
3. **Cross-token validation** — TOKEN_MISMATCH for task creation only (step 8).
4. **Payload-level signer check** — signer matches the `poster_id`/`bidder_id`/`worker_id` field in the payload itself (step 9a). This catches impersonation attempts before touching the database.
5. **Task lookup** — TASK_NOT_FOUND (step 10).
6. **Status check** — INVALID_STATUS (step 11). This MUST come before role-dependent checks.
7. **Role-dependent signer check** — signer is the task's actual poster/worker (step 9b). Only checked after the task is loaded and status is valid.
8. **Domain-specific validation** — SELF_BID, BID_ALREADY_EXISTS, NO_ASSETS, INVALID_REASON, INVALID_WORKER_PCT (step 12).
9. **External service calls** — CENTRAL_BANK_UNAVAILABLE (step 13).

### Status Check Before Role Check (Critical)

For operations where the role (worker_id) is only assigned in specific statuses (e.g., upload_asset requires ACCEPTED status for the worker to exist), the status check MUST come before the role check. Example: uploading to an OPEN task returns `409 INVALID_STATUS`, not `403 FORBIDDEN`, because there is no worker yet.

### Lazy Deadline Evaluation

Deadlines are evaluated on every read (`get_task`, `list_tasks`) and before every mutation that loads a task. The evaluation:

1. Checks if the current status has an active deadline that has passed.
2. Uses `UPDATE ... WHERE status = <current>` for atomic transition (prevents double-transition on concurrent requests).
3. Attempts escrow release after the transition.
4. If escrow release fails, sets `escrow_pending = true` and retries on the next read.

### Escrow Rollback on Task Creation

If the DB insert fails after a successful escrow lock (e.g., race condition on duplicate task_id), the service releases the escrow back to the poster before returning the error. If the rollback release also fails, it is logged but not propagated — the escrow will need manual recovery.

### Asset Storage

Files are stored at `{storage_path}/{task_id}/{asset_id}/{filename}`. The `get_asset` method resolves the file path and verifies it stays within the storage root to prevent path traversal attacks.

### SQLite Thread Safety

The database uses `check_same_thread=False` and WAL journal mode. Write atomicity for deadline transitions is ensured via `UPDATE ... WHERE status = <expected>` — only one concurrent request's update will match the condition.
...
- Phase 2 (`config.py`, `logging.py`, `schemas.py`)
- Phase 3 (`core/state.py`, `core/exceptions.py`)
- Phase 4 (`clients/identity_client.py`, `clients/central_bank_client.py`, `clients/platform_signer.py`)

The `TaskManager` constructor receives client instances injected from the lifespan (Phase 7). The service layer has no knowledge of how clients are created — it only uses their public interfaces.
