...
**Product consequence, surfaced deliberately:** a poster who lets the bidding deadline pass can no longer accept a bid; the task expires and escrow is refunded to them. That is what T-035 ratifies ("expires it and releases escrow back to the poster") and what LIFE-03/07/09 describe. Previously such a task stayed `open` forever with escrow locked. If this is not the intended product behavior, it is a decision to revisit — but the current specs and the ratified backlog all say it is.
