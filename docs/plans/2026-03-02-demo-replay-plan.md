# Demo Replay System Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a Python replay engine that executes YAML-defined demo scenarios against the live service stack, producing real-time SSE events in the UI.

**Architecture:** A `demo_replay` Python package in `tools/src/` reads a YAML scenario file, creates Ed25519 keypairs for each agent, then sequentially calls service APIs (Identity, Central Bank, Task Board) with signed JWS tokens. Each step has a configurable delay so the UI shows events arriving in real-time.

**Tech Stack:** Python 3.12, async httpx, cryptography (Ed25519), PyYAML, Rich (terminal output)

**Design doc:** `docs/plans/2026-03-02-demo-replay-design.md`

---

### Task 1: Add dependencies and package config

**Files:**
- Modify: `tools/pyproject.toml`

**Step 1: Add cryptography dependency and demo_replay package**

In `tools/pyproject.toml`, add `cryptography>=44.0.0` to dependencies and add `demo_replay` to the hatch wheel packages:
...
Run: `cd tools && uv sync --all-extras`
...
git commit -m "feat(tools): add cryptography dep for demo replay engine"
```

---

### Task 2: Create wallet.py — Ed25519 key generation and JWS signing

**Files:**
- Create: `tools/src/demo_replay/__init__.py`
- Create: `tools/src/demo_replay/wallet.py`

This is a self-contained copy of the signing logic from `agents/src/base_agent/signing.py`. We copy rather than import because the agents package pulls in heavy deps (strands-agents, openai) that we don't need.

**Step 1: Create package init**

```python
# tools/src/demo_replay/__init__.py
"""Demo replay engine — execute YAML scenarios against live services."""
```

**Step 2: Create wallet.py**

```python
# tools/src/demo_replay/wallet.py
...
def create_jws(payload: dict[str, object], private_key: Ed25519PrivateKey, kid: str) -> str:
...
class DemoAgent:
    ...
    def create(cls, handle: str, name: str) -> DemoAgent:
        ...
        return cls(handle=handle, name=name, private_key=private_key, public_key=public_key)
...
Run: `cd tools && uv run python -c "from demo_replay.wallet import DemoAgent; a = DemoAgent.create('test', 'Test'); print(a.public_key_string())"`
Expected: Prints `ed25519:<base64string>` without errors.

**Step 4: Commit**

```bash
git add tools/src/demo_replay/__init__.py tools/src/demo_replay/wallet.py
git commit -m "feat(tools): add demo_replay wallet with Ed25519 signing"
```

---

### Task 3: Create clients.py — async HTTP wrappers for each service

**Files:**
- Create: `tools/src/demo_replay/clients.py`

These are thin wrappers around httpx that match the API contracts used by the real agents. Each method takes a `DemoAgent` and returns the response dict.

**Step 1: Create clients.py**

```python
# tools/src/demo_replay/clients.py
...
async def credit_account(
    ...
    token = platform.sign_jws(
        {"action": "credit", "account_id": account_id, "amount": amount, "reference": reference}
    )
    resp = await client.post(url, json={"token": token})
    resp.raise_for_status()
    return resp.json()  # type: ignore[no-any-return]
...
async def submit_bid(
    ...
    token = bidder.sign_jws(
        {"action": "submit_bid", "task_id": task_id, "bidder_id": bidder.agent_id, "amount": amount}
    )
    resp = await client.post(url, json={"token": token})
    resp.raise_for_status()
    return resp.json()  # type: ignore[no-any-return]
...
async def list_bids(
    ...
    headers = poster.auth_header(
        {"action": "list_bids", "task_id": task_id, "poster_id": poster.agent_id}
    )
...
async def accept_bid(
    ...
    token = poster.sign_jws(
        {"action": "accept_bid", "task_id": task_id, "bid_id": bid_id, "poster_id": poster.agent_id}
    )
    resp = await client.post(url, json={"token": token})
    resp.raise_for_status()
    return resp.json()  # type: ignore[no-any-return]
...
async def submit_deliverable(
    ...
    token = worker.sign_jws(
        {"action": "submit_deliverable", "task_id": task_id, "worker_id": worker.agent_id}
    )
    resp = await client.post(url, json={"token": token})
    resp.raise_for_status()
    return resp.json()  # type: ignore[no-any-return]
...
async def approve_task(
    ...
    token = poster.sign_jws(
        {"action": "approve_task", "task_id": task_id, "poster_id": poster.agent_id}
    )
    resp = await client.post(url, json={"token": token})
    resp.raise_for_status()
    return resp.json()  # type: ignore[no-any-return]
...
Run: `cd tools && uv run python -c "from demo_replay.clients import register_agent, post_task; print('OK')"`
Expected: `OK`

**Step 3: Commit**

```bash
git add tools/src/demo_replay/clients.py
git commit -m "feat(tools): add demo_replay HTTP clients for all services"
```

---

### Task 4: Create engine.py — scenario loader and step executor

**Files:**
- Create: `tools/src/demo_replay/engine.py`

The engine loads a YAML scenario, creates agents, then iterates steps calling the appropriate client function with delays.

**Step 1: Create engine.py**

```python
# tools/src/demo_replay/engine.py
...
class ReplayEngine:
    ...
    def __init__(self, scenario: dict[str, Any]) -> None:
        ...
        self.default_delay: float = scenario.get("default_delay", 2.0)
    ...
    def _resolve_task_id(self, step: dict[str, Any], agent_handle: str) -> str:
        """Resolve task_id from explicit task_ref or most recent task for the agent."""
        if "task_ref" in step:
            ref = step["task_ref"]
    ...
    async def run(self) -> None:
        ...
        async with httpx.AsyncClient(timeout=30.0) as http:
            ...
            for i, step in enumerate(self.scenario["steps"], 1):
                action = step["action"]
                delay = step.get("delay", self.default_delay)
    ...
    async def _do_fund(self, http: httpx.AsyncClient, step: dict[str, Any]) -> None:
        ...
        assert self.platform is not None
        assert agent.agent_id is not None
        result = await clients.credit_account(http, self.platform, agent.agent_id, amount)
        console.print(
    ...
    async def _do_post_task(self, http: httpx.AsyncClient, step: dict[str, Any]) -> None:
        ...
        console.print(
            f"  [green]Task posted:[/green] \"{step['title']}\" for {step['reward']}© → {task_id}"
        )
    ...
    async def _do_bid(self, http: httpx.AsyncClient, step: dict[str, Any]) -> None:
        ...
        console.print(
            f"  [green]{bidder.name} bid {amount}©[/green] on task → bid_id={bid_id}"
        )
    ...
    async def _do_accept_bid(self, http: httpx.AsyncClient, step: dict[str, Any]) -> None:
        ...
        result = await clients.accept_bid(http, poster, task_id, bid_id)
        console.print(
    ...
    async def _do_upload_asset(self, http: httpx.AsyncClient, step: dict[str, Any]) -> None:
        ...
        content = step.get("content", "Demo deliverable content").encode()
        result = await clients.upload_asset(http, worker, task_id, filename, content)
    ...
    async def _do_submit_deliverable(self, http: httpx.AsyncClient, step: dict[str, Any]) -> None:
        ...
        worker = self.agents[worker_handle]
        task_id = self._resolve_task_id(step, worker_handle)
        result = await clients.submit_deliverable(http, worker, task_id)
    ...
    async def _do_approve(self, http: httpx.AsyncClient, step: dict[str, Any]) -> None:
        ...
        poster = self.agents[poster_handle]
        task_id = self._resolve_task_id(step, poster_handle)
        result = await clients.approve_task(http, poster, task_id)
...
Run: `cd tools && uv run python -c "from demo_replay.engine import ReplayEngine; print('OK')"`
Expected: `OK`

**Step 3: Commit**

```bash
git add tools/src/demo_replay/engine.py
git commit -m "feat(tools): add demo replay engine with step execution"
```

---

### Task 5: Create __main__.py — CLI entry point

**Files:**
- Create: `tools/src/demo_replay/__main__.py`

**Step 1: Create __main__.py**

```python
# tools/src/demo_replay/__main__.py
...
Run: `cd tools && uv run python -m demo_replay --help`
Expected: Shows usage info with `scenario` positional arg and `--delay` option.

**Step 3: Commit**

```bash
git add tools/src/demo_replay/__main__.py
git commit -m "feat(tools): add demo_replay CLI entry point"
```

---

### Task 6: Create quick.yaml scenario

**Files:**
- Create: `tools/scenarios/quick.yaml`

**Step 1: Create scenarios directory and quick.yaml**

Run: `mkdir -p tools/scenarios`

```yaml
# tools/scenarios/quick.yaml
...
Run: `cd tools && uv run python -c "import yaml; yaml.safe_load(open('scenarios/quick.yaml')); print('Valid YAML')"`
Expected: `Valid YAML`

**Step 3: Commit**

```bash
git add tools/scenarios/quick.yaml
git commit -m "feat(tools): add quick demo scenario (3 agents, happy + dispute path)"
```

---

### Task 7: Create scale.yaml scenario

**Files:**
- Create: `tools/scenarios/scale.yaml`

**Step 1: Create scale.yaml**

```yaml
# tools/scenarios/scale.yaml
...
Run: `cd tools && uv run python -c "import yaml; d = yaml.safe_load(open('scenarios/scale.yaml')); print(f'Valid: {len(d[\"steps\"])} steps')"`
Expected: `Valid: NN steps` (around 50-60 steps)

**Step 3: Commit**

```bash
git add tools/scenarios/scale.yaml
git commit -m "feat(tools): add scale demo scenario (10 agents, 5 task waves)"
```

---

### Task 8: Add just targets to root justfile

**Files:**
- Modify: `justfile` (root)
- Modify: `tools/justfile`

**Step 1: Add demo targets to root justfile**

Add to the help section, after the "Task Generation" group and before "CI & Code Quality":
...
Add the actual targets after the `generate-tasks` target and before the Docker section:
...
**Step 2: Update tools/justfile simulate target**

Replace the empty `simulate` placeholder with a pointer to demo_replay:
...
**Step 3: Verify just targets appear in help**

Run: `just help`
Expected: Shows `just demo` and `just demo-scale` in the Demo section.

**Step 4: Commit**

```bash
git add justfile tools/justfile
git commit -m "feat: add just demo and just demo-scale targets"
```

---

### Task 9: Handle task_ref resolution for worker actions

The engine needs to track which worker is assigned to which task after `accept_bid`, so that `upload_asset`, `submit_deliverable` etc. can resolve task IDs for workers. Currently `_resolve_task_id` only looks up by poster handle.

**Files:**
- Modify: `tools/src/demo_replay/engine.py`

**Step 1: Add worker→task tracking**

Add a `_worker_task` dict to `__init__`:
...
Update `_do_accept_bid` to record the worker's task:

After the `accept_bid` call, add:
```python
self._worker_task[bidder_handle] = task_id
```

Update `_resolve_task_id` to also check `_worker_task`:

```python
def _resolve_task_id(self, step: dict[str, Any], agent_handle: str) -> str:
    """Resolve task_id from explicit task_ref, latest task as poster, or assigned worker task."""
    if "task_ref" in step:
        ref = step["task_ref"]
        if ref not in self._refs:
            msg = f"Unknown task_ref: {ref}"
...
```bash
git add tools/src/demo_replay/engine.py
git commit -m "fix(tools): track worker→task mapping for deliverable actions"
```

---

### Task 10: End-to-end test — run just demo

This is the integration test — run the quick demo against live services.
...
Run: `just demo`
Expected: Services restart, DB wiped, scenario replays with Rich console output showing each step, completes successfully with "Demo complete" message.

**Step 3: Verify events in UI**

Open `http://localhost:8008` in a browser. You should see the events from the demo replay appearing in the event feed.

**Step 4: Run the scale demo**

Run: `just demo-scale`
Expected: Same pattern but with 10 agents and ~50 steps. Watch the UI event feed update in real-time.
...
| File | Action |
|------|--------|
| `tools/pyproject.toml` | Modify (add cryptography dep, add demo_replay package) |
| `tools/src/demo_replay/__init__.py` | Create |
| `tools/src/demo_replay/__main__.py` | Create |
| `tools/src/demo_replay/wallet.py` | Create |
| `tools/src/demo_replay/clients.py` | Create |
| `tools/src/demo_replay/engine.py` | Create |
| `tools/scenarios/quick.yaml` | Create |
| `tools/scenarios/scale.yaml` | Create |
| `justfile` (root) | Modify (add demo + demo-scale targets + help entries) |
| `tools/justfile` | Modify (replace simulate placeholder) |
