"""Architecture test fixtures for central-bank service."""
...
_PKG_DIR = _SERVICE_ROOT / "src" / "central_bank_service"
_REPORTS_DIR = _SERVICE_ROOT / "reports" / "architecture"


@pytest.fixture(scope="session")
def evaluable() -> EvaluableArchitecture:
    """Build the evaluable architecture graph for central_bank_service."""
...
def layered_arch() -> LayeredArchitecture:
    ...
    return (
        LayeredArchitecture()
        .layer("routers")
        .containing_modules(["central_bank_service.routers"])
        .layer("core")
        .containing_modules(["central_bank_service.core"])
        .layer("services")
        .containing_modules(["central_bank_service.services"])
    )
