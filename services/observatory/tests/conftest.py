...
def seeded_db_path(tmp_path: Path, schema_sql: str) -> Path:
    ...
    conn.executemany(
        ...
        [
            ...
            ("esc-1", "a-alice", 100, "t-1",
             "released", _ts(t_2h30m), _ts(t_2h)),
            # t-2: released (approved, Charlie got paid)
            ("esc-2", "a-alice", 50, "t-2",
             "released", _ts(t_1h30m), _ts(t_1h)),
            # t-3: locked (in execution)
            ("esc-3", "a-bob", 80, "t-3",
             "locked", _ts(t_45m), None),
            # t-4: locked (open for bidding)
            ("esc-4", "a-charlie", 60, "t-4",
             "locked", _ts(t_15m), None),
            # t-5: split (disputed + ruled, worker_pct=70)
            ("esc-5", "a-alice", 120, "t-5",
             "split", _ts(t_2h30m), _ts(t_30m)),
        ],
    )
    ...
    _insert_task(conn, (
        "t-1", "a-alice", "Build login page",
        "Create a responsive login page with OAuth",
        100, "approved",
        3600, 7200, 3600,
        _ts(t_2h30m + bd), _ts(t_2h30m + ex), _ts(t_2h30m + rv),
        "esc-1", "a-bob", "bid-1",
        None, None, None, None,
        _ts(t_2h30m), _ts(t_2h30m + timedelta(minutes=10)),
        _ts(t_2h - timedelta(minutes=10)), _ts(t_2h),
        None, None, None, None,
    ))

    # t-2: Alice -> Charlie, reward 50, approved 1h ago
    _insert_task(conn, (
        "t-2", "a-alice", "Write unit tests",
        "Write comprehensive unit tests for auth module",
        50, "approved",
        3600, 7200, 3600,
        _ts(t_1h30m + bd), _ts(t_1h30m + ex), _ts(t_1h30m + rv),
        "esc-2", "a-charlie", "bid-4",
        None, None, None, None,
        _ts(t_1h30m), _ts(t_1h30m + timedelta(minutes=10)),
        _ts(t_1h - timedelta(minutes=10)), _ts(t_1h),
        None, None, None, None,
    ))

    # t-3: Bob -> Charlie, reward 80, accepted 30m ago
    _insert_task(conn, (
        "t-3", "a-bob", "Design API schema",
        "Design RESTful API schema for task board",
        80, "accepted",
        3600, 7200, 3600,
        _ts(t_45m + bd), _ts(t_30m + ex), None,
        "esc-3", "a-charlie", "bid-6",
        None, None, None, None,
        _ts(t_45m), _ts(t_30m),
        None, None,
        None, None, None, None,
    ...
    _insert_task(conn, (
        "t-4", "a-charlie", "Implement caching",
        "Add Redis caching layer for frequent data",
        60, "open",
        3600, 7200, 3600,
        _ts(t_15m + bd), None, None,
        "esc-4", None, None,
        None, None, None, None,
        _ts(t_15m), None,
        None, None,
    ...
    ruling_reason = (
        "Deliverable does not meet spec requirements "
        "for connection pooling"
    )
    ruling_summary = (
        "Worker completed 70% of requirements. "
        "Connection pooling was partially implemented."
    )
    _insert_task(conn, (
        "t-5", "a-alice", "Refactor database layer",
        "Refactor the DB layer to use connection pooling",
        120, "ruled",
        3600, 7200, 3600,
        _ts(t_2h30m + bd), _ts(t_2h30m + ex), _ts(t_2h30m + rv),
        "esc-5", "a-bob", "bid-7",
        ruling_reason, "rul-1", 70, ruling_summary,
        _ts(t_2h30m), _ts(t_2h30m + timedelta(minutes=10)),
        _ts(t_1h), None,
        None, _ts(t_45m), _ts(t_30m), None,
    ))
    ...
    conn.executemany(
        ...
        [
            ...
            ("fb-1", "t-1", "a-alice", "a-bob",
             "poster", "delivery_quality", "satisfied",
             "Good work on the login page",
             _ts(t_2h + timedelta(minutes=5)), 1),
            ("fb-2", "t-1", "a-bob", "a-alice",
             "worker", "spec_quality", "extremely_satisfied",
             "Clear and detailed spec",
             _ts(t_2h + timedelta(minutes=6)), 1),
            # t-2 visible (both submitted -> revealed)
            ("fb-3", "t-2", "a-alice", "a-charlie",
             "poster", "delivery_quality", "extremely_satisfied",
             "Excellent test coverage",
             _ts(t_1h + timedelta(minutes=5)), 1),
            ("fb-4", "t-2", "a-charlie", "a-alice",
             "worker", "spec_quality", "satisfied",
             "Spec was reasonable",
             _ts(t_1h + timedelta(minutes=6)), 1),
            # t-5 sealed (dispute scenario)
            ("fb-5", "t-5", "a-alice", "a-bob",
             "poster", "delivery_quality", "dissatisfied",
             "Did not meet requirements",
             _ts(t_45m + timedelta(minutes=2)), 0),
            ("fb-6", "t-5", "a-bob", "a-alice",
             "worker", "spec_quality", "dissatisfied",
             "Spec was unclear about pooling requirements",
             _ts(t_45m + timedelta(minutes=3)), 0),
        ],
    )
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        ("clm-1", "t-5", "a-alice", "a-bob",
         ruling_reason, "ruled", _ts(t_45m)),
    )
    ...
    judge_votes = json.dumps([
        ...
        {"judge": "judge-2", "worker_pct": 65,
         "reason": "Missing pool limits"},
        {"judge": "judge-3", "worker_pct": 75,
         "reason": "Core functionality present"},
    ])
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        ("rul-1", "clm-1", "t-5", 70,
         ruling_summary, judge_votes, _ts(t_30m)),
    )
    ...
    conn.executemany(
        ...
        [
            # Salary credits
            ("tx-1", "a-alice", "credit",
             1000, 1000, "salary_round_1", _ts(t_3h)),
            ("tx-2", "a-bob", "credit",
             1000, 1000, "salary_round_1", _ts(t_3h)),
            ("tx-3", "a-charlie", "credit",
             1000, 1000, "salary_round_1", _ts(t_3h)),
            # Alice locks for t-1
            ("tx-4", "a-alice", "escrow_lock",
             100, 900, "t-1", _ts(t_2h30m)),
            # Alice locks for t-5
            ("tx-5", "a-alice", "escrow_lock",
             120, 780, "t-5",
             _ts(t_2h30m + timedelta(minutes=1))),
            # Bob receives t-1 payment
            ("tx-6", "a-bob", "escrow_release",
             100, 1100, "t-1", _ts(t_2h)),
            # Alice locks for t-2
            ("tx-7", "a-alice", "escrow_lock",
             50, 730, "t-2", _ts(t_1h30m)),
            # Charlie receives t-2 payment
            ("tx-8", "a-charlie", "escrow_release",
             50, 1050, "t-2", _ts(t_1h)),
            # Bob locks for t-3
            ("tx-9", "a-bob", "escrow_lock",
             80, 1020, "t-3", _ts(t_45m)),
            # t-5 ruling split: Bob 84, Alice 36
            ("tx-10", "a-bob", "escrow_release",
             84, 1104, "t-5", _ts(t_30m)),
            ("tx-11", "a-alice", "escrow_release",
             36, 766, "t-5", _ts(t_30m)),
            # Charlie locks for t-4
            ("tx-12", "a-charlie", "escrow_lock",
             60, 990, "t-4", _ts(t_15m)),
        ],
    )
    ...
    conn.executemany(
        ...
        [
            ...
            ("identity", "agent.registered", _ts(t_3h),
             None, "a-alice", "Alice registered as an agent",
             json.dumps({"agent_name": "Alice"})),
            ("identity", "agent.registered", _ts(t_3h),
             None, "a-bob", "Bob registered as an agent",
            ...
            # 4: Task t-1 created
            ("board", "task.created", _ts(t_2h30m),
             "t-1", "a-alice",
            ...
            # 8: Task t-2 created
            ("board", "task.created", _ts(t_1h30m),
             "t-2", "a-alice",
            ...
            ("board", "task.created", _ts(t_2h30m),
             "t-5", "a-alice",
             "Alice posted 'Refactor database layer' "
             "for 120 tokens",
             json.dumps({
            ...
            # 14: Ruling delivered
            ("court", "ruling.delivered", _ts(t_30m),
             "t-5", None,
            ...
            ("board", "task.created", _ts(t_15m),
             "t-4", "a-charlie",
             "Charlie posted 'Implement caching' "
             "for 60 tokens",
             json.dumps({
