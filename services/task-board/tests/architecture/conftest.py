"""Architecture test fixtures for task-board service."""
...
_PKG_DIR = _SERVICE_ROOT / "src" / "task_board_service"
_REPORTS_DIR = _SERVICE_ROOT / "reports" / "architecture"


@pytest.fixture(scope="session")
def evaluable() -> EvaluableArchitecture:
    """Build the evaluable architecture graph for task_board_service."""
...
def layered_arch() -> LayeredArchitecture:
    ...
    return (
        LayeredArchitecture()
        .layer("routers")
        .containing_modules(["task_board_service.routers"])
        .layer("core")
