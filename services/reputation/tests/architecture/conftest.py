"""Architecture test fixtures for reputation service."""
...
_PKG_DIR = _SERVICE_ROOT / "src" / "reputation_service"
_REPORTS_DIR = _SERVICE_ROOT / "reports" / "architecture"


@pytest.fixture(scope="session")
def evaluable() -> EvaluableArchitecture:
    """Build the evaluable architecture graph for reputation_service."""
...
def layered_arch() -> LayeredArchitecture:
    ...
    return (
        LayeredArchitecture()
        .layer("routers")
        .containing_modules(["reputation_service.routers"])
        .layer("core")
        .containing_modules(["reputation_service.core"])
        .layer("services")
        .containing_modules(["reputation_service.services"])
    )
